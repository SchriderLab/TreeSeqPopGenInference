...
def plot_roc(y_true, y_probs, schema, scenarios, outfile):
    for i1, s1 in enumerate(scenarios[1:], 1):
        for i2, s2 in enumerate(scenarios[1:], 1):
            if i1 != i2:
                ...
                swp_fpr, swp_tpr, thresh = roc_curve(
                    sweep_labs, pos_probs, pos_label=i2
                )
    ...
    plt.plot(
        fpr,
        tpr,
        label=f"{schema.capitalize()} {scenarios[0].upper()} vs Sweep AUC: {auc_val:.2}",
    )
...
def plot_prec_recall(y_true, y_probs, schema, scenarios, outfile):
    for i1, s1 in enumerate(scenarios[1:], 1):
        for i2, s2 in enumerate(scenarios[1:], 1):
            if i1 != i2:
                # Plot sdn/ssv distinction
                ...
                swp_prec, swp_rec, thresh = precision_recall_curve(
                    sweep_labs, pos_probs, pos_label=i2
                )
