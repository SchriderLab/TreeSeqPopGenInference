    ...
    def __getitem__(self, index):
        ...
        for ix in range(self.n_per):
            ...
            x1 = torch.FloatTensor(
                (np.array(self.ifile[key]["x1"]) - self.x1_mean) / self.x1_std
            )
...
class AutoGenerator(object):
    def __init__(
        ...
        D_means="./D_mean_stuff_v2/D_mean.npz",
        n_samples_per=20,
        chunk_size=5,
        ...
        self.D_std[np.where(self.D_std == 0.0)] = 1.0
    ...
    def __getitem__(self, index):
        ...
        batch = Batch.from_data_list(
            [
                Data(
                    x=torch.FloatTensor(x[k]),
                    edge_index=torch.LongTensor(edge_index[k]),
                )
