...
def main():
    ...
    for ik in range(len(ifiles)):
        ...
        if len(Xs) > 0:
            if ik < N:
                ofile.create_dataset('{1}/{0}/x'.format(ik, tag), data = np.array(Xs), compression = 'lzf')
                ofile.create_dataset('{1}/{0}/edge_index'.format(ik, tag), data = np.array(Edges).astype(np.int32), compression = 'lzf')
                ofile.create_dataset('{1}/{0}/info'.format(ik, tag), data = np.array(infos), compression = 'lzf')
            else:
                ofile_val.create_dataset('{1}/{0}/x'.format(ik - N, tag), data = np.array(Xs), compression = 'lzf')
                ofile_val.create_dataset('{1}/{0}/edge_index'.format(ik - N, tag), data = np.array(Edges).astype(np.int32), compression = 'lzf')
                ofile_val.create_dataset('{1}/{0}/info'.format(ik - N, tag), data = np.array(infos), compression = 'lzf')
        
        Xg = x[0]
        A = np.array(As)
        
        if ik < N:
            ofile.create_dataset('{1}/{0}/x_0'.format(ik, tag), data = Xg.astype(np.uint8), compression = 'lzf')
            if A.shape[0] > 0:
                ofile.create_dataset('{1}/{0}/A'.format(ik, tag), data = A, compression = 'lzf')
            ofile.flush()
        else:
            ofile_val.create_dataset('{1}/{0}/x_0'.format(ik - N, tag), data = Xg.astype(np.uint8), compression = 'lzf')
            if A.shape[0] > 0:
                ofile_val.create_dataset('{1}/{0}/A'.format(ik - N, tag), data = A, compression = 'lzf')
            ofile_val.flush()
